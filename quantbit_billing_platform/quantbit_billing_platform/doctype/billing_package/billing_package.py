...
class BillingPackage(Document):
    ...
    def before_save(self):
        ...
        if not self.is_user_package and not self.is_base_package:
            ...
            if not user_package:
                frappe.throw(f"User package must be created first for app '{self.app_name}'")

            if not base_package:
                frappe.throw(f"Base package must be created first for app '{self.app_name}'")
        ...
        if self.is_user_package:
            ...
            if existing:
                frappe.throw(f"User package already exists for app '{self.app_name}'")
        ...
        if self.is_base_package:
            ...
            if existing:
                frappe.throw(f"Base package already exists for app '{self.app_name}'")
